...
| Project follow-ups                 | Implemented     | Teacher/admin project follow-ups are filtered in SQL before the 16-row cap so pending-review projects and non-follow-up rows cannot starve stale/missing-progress candidates. |
| Student assignments                | Implemented     | Student dashboard shows active-project assigned work grouped into `Needs revision`, `Overdue`, and `Next up`, excluding completed assignments and assignments already waiting for review. |
...
| Assignment-scoped progress         | Implemented     | Dashboard review queues, recent submissions, pending-review stats, and stale-progress ordering ignore historical child-task progress. |
| Feature audit                      | Audited A12 2026-06-20 | Reviewed documented intent, backend role scoping and lifecycle filters, frontend UI/UX, cache behavior, Playwright role evidence, and targeted verification. |
| Admin-specific dashboard UI        | Implemented     | Admin uses global review-queue copy and compact teacher/student count chips; deeper admin analytics remain future admin-basics work. |
| Frontend dashboard tests           | Partial         | `apps/web/e2e/dashboard.spec.ts` covers role branches, admin count chips, teacher review/overdue queues, and student revision/overdue queues. |
...
| Student opens `/dashboard`                    | Sees actionable assignments grouped by revision/overdue/next-up urgency, then recent submissions below. |
| Admin opens `/dashboard`                      | Sees global review queue copy, global review/overdue/project chips, teacher/student count chips, and global review/attention data. |
...
| Teacher clicks a review item                  | Navigates to `/workspace/projects/:projectId/tasks/:taskId#assignment-workflow`.                |
...
| `projects`        | `Project[]`                   | Students receive up to 16 joined projects; teachers/admins receive up to 16 active no-pending-review follow-up candidates. |
...
| `pendingReviews`   | Admin/teacher: assignment-scoped pending project progress reviews excluding archived projects. Student: student's assignment submissions still pending review. |
| `studentCount`     | Admin: all student users. Teacher: distinct students across supervised projects. Rendered as compact summary chips for admin/teacher. |
| `teacherCount`     | Admin: all teacher users. Rendered as a compact summary chip for admin. Omitted for teachers/students. |

The frontend renders compact role-aware summary chips instead of a generic KPI-card strip. Student chips focus on assigned work; teacher/admin chips focus on review debt, overdue work, project count, and visible people counts where relevant.

## Role-Scoped Data Matrix

| Role    | Stats Scope             | Projects Payload                                  | Tasks Payload                              | Progress Payload                                 |
| ------- | ----------------------- | ------------------------------------------------- | ------------------------------------------ | ------------------------------------------------ |
| Admin   | Global                  | Up to 16 global active no-pending-review follow-up candidates | Up to 8 global overdue active-project assignments | Up to 8 non-archived assignment-scoped global pending reviews, oldest first |
| Teacher | Supervised projects     | Up to 16 supervised active no-pending-review follow-up candidates | Up to 8 supervised overdue active-project assignments | Up to 8 non-archived assignment-scoped supervised pending reviews, oldest first |
| Student | Memberships/assignments | Up to 16 joined projects ordered by project update | Up to 8 assigned active-project assignments that are not completed and not waiting for review | Up to 8 own assignment-scoped submissions, newest first |
...
| `task_assignees`   | `project_id`, `task_id`, `student_id`                                                               | Student assignment counts and assignment list scoped to current project assignments. |
| `progress_updates` | `id`, `project_id`, `task_id`, `submitted_by`, `review_status`, `created_at`                        | Review queue and recent submission list; student submission rows require current project membership. |
...
| `apps/api/internal/app/lifecycle_test.go` | Backend regression coverage for protected access, pending review ordering, attention order, project follow-up starvation, and student actionable assignments. |
...
| `dashboardProjects`        | Returns student joined projects or teacher/admin follow-up candidates filtered before caps.      |
...
| `apps/web/src/features/dashboard/pages/dashboard-page.tsx`     | Renders loading/error states, role-aware compact summary chips, student actionable queue, teacher/admin review queue, overdue assignment follow-ups, and project follow-ups without a generic KPI card strip. |
...
| `apps/web/e2e/dashboard.spec.ts`                               | Browser regression for admin count chips, teacher review/overdue queues, and student revision/overdue queues. |
...
flowchart TD
  ...
  E --> F[Each review links to the assignment review desk]
  A --> G[Render overdue assignment rows]
  A --> H[Receive active no-pending-review follow-up candidates]
  H --> I[Frontend applies projectNeedsAttention as safety filter]
  I --> K[Render project follow-up]
```
...
flowchart TD
  ...
  D -- Yes --> E[Group work into Needs revision, Overdue, Next up]
  E --> G[Render grouped assignment tables with project context]
  D -- No --> F[Show assignment empty state]
  A --> H[Recent submissions section]
  H --> I{Submissions present?}
  I -- Yes --> J[Render ProgressUpdateTable]
  I -- No --> K[Show submissions empty state]
```

## Attention Ordering

Teacher/admin dashboard projects are filtered and ordered in the backend before the 16-row cap:

1. Active projects only.
2. Assignment-scoped pending-review projects are excluded because the review queue already covers them.
3. Candidates must have overdue assignments or at least one assignment with missing/stale approved progress.
4. More overdue assignments first.
5. Missing or stale approved progress next.
6. Oldest or missing approved review date first.
7. Most recently updated project first as a final tie-breaker.

The frontend `projectNeedsAttention` check remains as a safety filter for active projects with overdue assignments, pending reviews, no approved update, or approved progress older than seven days. The backend now applies the dashboard follow-up candidate rules before `LIMIT`, so review-heavy projects or empty non-follow-up projects cannot starve the project follow-up section.
...
| `TestTeacherDashboardProjectFollowUpsAreNotStarvedByPendingReviews` | Pending-review projects cannot consume the project payload cap before stale follow-up projects are returned. |
...
| Test | Coverage |
| --- | --- |
| `role dashboards render actionable queues and admin counts` | API-created fixture verifies admin teacher/student count chips, teacher review/overdue queues, and student needs-revision/overdue queues. |

Frontend tests for dashboard loading, request failures, fully empty role states, and review-link anchors are still missing.
...
| Project attention is split between backend candidate filtering and frontend safety filtering | Keep `dashboardProjects` follow-up rules and `projectNeedsAttention` semantics aligned.              |
...
| Frontend tests are still partial                                           | Role branches have Playwright coverage; loading, error, empty, and review-anchor regressions can still slip through. |
| Admin dashboard remains intentionally lightweight                          | Admin gets global work queues and people-count chips, but all-project management, activity logs, and richer admin analytics are separate admin-basics work. |
...
- Keep teacher/admin project follow-up candidate filtering in SQL before `LIMIT`.
...
- Preserve student scope through `project_members`, project-aware `task_assignees`, and `submitted_by`; old submissions should not expose live project metadata after membership removal.
