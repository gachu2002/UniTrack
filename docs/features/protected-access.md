...
- Project-scoped writes also pass lifecycle gates based on `project.status` after relationship permission succeeds, then existing mutation transactions re-check status under a project-row lock before writing.
...
| Project lifecycle write gates    | Implemented     | Active/on-hold/completed/archived status controls project-scoped mutations after relationship checks, with transaction-scoped project-row lock rechecks for existing write routes. |
...
| Frontend route/access tests      | Partial         | Playwright covers key route denials, hidden manager actions, inactive login blocking, non-member project denial, and closed-project evidence affordances; broader route/component tests are still needed. |
...
| Project lifecycle gate  | A status-specific write check run after project relationship authorization and rechecked under lock before mutation. |
...
| `task_assignees`          | `project_id`, `task_id`, `student_id`                        | Assigned-student submission access scoped to project membership. |
| `resource_links`          | `project_id`, `added_by`, target fields                      | Resource write/delete authorization.                   |
| `uploaded_files`          | `project_id`, `uploaded_by`, `related_entity_type`, `related_entity_id` | File download/delete/upload authorization.             |
...
| `apps/web/e2e/access-control.spec.ts` | Browser regression coverage for non-admin route denial, student folder denial, hidden project manager actions, non-member project denial, inactive login blocking, and closed-project evidence affordances. |
...
| `requireProjectLifecycle`            | Performs early status-specific project write preflight checks after view/manage checks pass.   |
| `requireProjectLifecycleTx` / `lockProjectLifecycleTx` | Re-checks project lifecycle under `SELECT ... FOR UPDATE` inside mutation transactions.      |
...
| `TestLifecycleLockRejectsAssignmentAfterConcurrentCompletion` | Assignment creation waits on a project-row lock and rejects after a concurrent completion.     |
| `TestLifecycleLockRejectsMetadataUpdateAfterConcurrentArchive` | Project metadata update waits on a project-row lock and rejects after a concurrent archive.    |
...
Frontend Playwright coverage in `apps/web/e2e/access-control.spec.ts` provisions temporary users/project data through the API and verifies key forbidden states, client affordances, inactive login blocking, non-member denial, and closed-project evidence read-only behavior. Broader route/component coverage is still needed for project/task form flows, team changes, redirects, and resource dialogs.
...
| Lifecycle gates are per-handler | New write handlers must call both the preflight lifecycle helper and the transaction-scoped lock helper with the correct allowed status set. |
...
| Frontend route/component tests are still partial | The access-control spec covers key regressions, but broader project/task forms, team flows, resource dialogs, and redirect permutations still need browser or component tests. |
...
- For project-scoped writes, call `requireProjectLifecycle` after relationship checks for preflight UX and `requireProjectLifecycleTx` / `lockProjectLifecycleTx` inside the mutation transaction before writing data.
