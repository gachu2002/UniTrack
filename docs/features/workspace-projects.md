...
Project headers expose concise folder, timeline, and team context through metadata pills. The Work Plan is visually introduced as the project sequence; its checkpoint cards use a subtle current-state emphasis, and Snapshot is the primary rail surface for planned progress.
