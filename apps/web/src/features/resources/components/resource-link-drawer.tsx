...
import { invalidateProjectSupportData } from '@/lib/query-invalidation'
...
export function ResourceLinkDialog({ projectId, target, resources, canCreate, canManageAll, readOnlyMessage, onClose }: ResourceLinkDialogProps) {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      ...
      refreshResourceStateOnStaleError(queryClient, projectId, error)
    },
  })
      ...
      {target ? (
        <div className="space-y-4 pb-4">
          ...
          {canCreate ? <div className="sticky bottom-0 -mx-5 border-t border-border bg-card/95 px-5 py-4 backdrop-blur sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            {editing ? (
              <ResourceLinkForm key={`edit-${editing.id}`} projectId={projectId} target={target} resource={editing} onSaved={() => setEditing(null)} onCancel={() => setEditing(null)} />
            ) : createOpen ? (
              <ResourceLinkForm key={`create-${target.type}-${target.id}`} projectId={projectId} target={target} onSaved={() => setCreateOpen(false)} onCancel={() => setCreateOpen(false)} />
            ) : (
    ...
    <article className="border-b border-border pb-3 last:border-b-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          ...
          {canManage ? <Button type="button" variant="edit" size="sm" onClick={onEdit} aria-label={`Edit resource ${resource.title}`}><Pencil className="size-4" /> Edit</Button> : null}
          {canManage ? <Button type="button" variant="ghost" size="sm" disabled={isDeleting} onClick={onDelete} aria-label={`Delete resource ${resource.title}`}><Trash2 className="size-4" /></Button> : null}
        </div>
      </div>
    </article>
  )
...
export function ResourceLinkButton({ count, onClick, ariaLabel = 'Manage resources' }: { count: number; onClick: () => void; ariaLabel?: string }) {
  return (
    <button type="button" className="inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-semibold text-muted-foreground transition hover:bg-muted hover:text-primary" onClick={onClick} aria-label={ariaLabel}>
  ...
  return (
    <section className={compact ? 'space-y-2' : 'rounded-2xl border border-border bg-card px-4 py-3'}>
      <div className="flex items-center justify-between gap-3">
        ...
        {onManage ? <Button type="button" variant="ghost" size="sm" onClick={onManage} aria-label={`${resources.length > 0 ? 'Manage' : 'Add'} ${title.toLowerCase()}`}>{resources.length > 0 ? 'Manage' : 'Add'}</Button> : null}
      </div>
      {resources.length > 0 ? (
...
function refreshResourceStateOnStaleError(queryClient: QueryClient, projectId: string, error: unknown) {
  if (!isForbiddenOrConflictError(error)) {
    return
  }
  invalidateProjectSupportData(queryClient, projectId)
}
