...
function ProjectCardContent({ project, showContext, context, hasAttention, linkedTitle, actions, palette }: { project: Project; showContext: boolean; context: string; hasAttention: boolean; linkedTitle: boolean; actions?: ReactNode; palette: ReturnType<typeof projectPalette> }) {
  ...
  const scopeLabel = project.milestoneCount > 0 ? `${project.completedMilestoneCount}/${project.milestoneCount} milestones` : `${project.completedTaskCount}/${project.taskCount} ${assignmentLabel}`
  ...
  return (
    <div className="relative">
      ...
      <div className={cn('relative overflow-hidden rounded-[1.15rem] rounded-tl-[0.75rem] border shadow-sm ring-1 ring-transparent transition duration-200 group-hover:shadow-panel group-hover:ring-primary/15 group-focus-within:ring-primary/20', compact ? 'min-h-36' : 'min-h-44', palette.card)}>
        ...
        <div className={cn('relative flex flex-col justify-between', compact ? 'min-h-36 gap-3 p-3' : 'min-h-44 gap-4 p-4')}>
