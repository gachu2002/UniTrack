...
import { refreshProjectDataOnStaleError, refreshWorkspaceDataOnStaleError } from '@/lib/query-invalidation'
import { queryKeys } from '@/lib/query-keys'
import { useAuthStore } from '@/stores/auth-store'
import type { ClassFolder, Project, ProjectMember } from '@/types/api'
...
function formatClassOption(item: ClassFolder) {
  return item.status === 'archived' ? `${item.title} (archived)` : item.title
}

function filterClassesForSupervisor(classes: ClassFolder[], supervisorId?: string, currentClassId?: string) {
  if (!supervisorId) {
    return []
  }
  return classes.filter((item) => (item.ownerTeacherId === supervisorId || item.id === currentClassId) && (item.status === 'active' || item.id === currentClassId))
}
...
export function AddProjectMemberForm({ projectId, onAdded }: { projectId: string; onAdded?: () => void }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (member) => {
      ...
      queryClient.setQueryData<ProjectMember[]>(queryKeys.projectMembers(projectId), (current) => current && current.some((existing) => existing.id === member.id) ? current : [...(current || []), member])
      queryClient.setQueryData<Project>(queryKeys.project(projectId), (current) => current ? { ...current, memberCount: current.memberCount + 1 } : current)
  ...
  return (
    <form className="space-y-3" onSubmit={form.handleSubmit((values) => mutation.mutate({ projectId, email: values.email }))}>
      <div>
        <FieldLabel htmlFor="project-member-email">Student email</FieldLabel>
        <Input id="project-member-email" type="email" placeholder="student@university.edu" aria-invalid={Boolean(form.formState.errors.email)} aria-describedby="project-member-email-help project-member-email-error" {...form.register('email')} />
        {form.formState.errors.email ? <p id="project-member-email-error" className="mt-1 text-xs font-semibold text-destructive">{form.formState.errors.email.message}</p> : null}
        <p id="project-member-email-help" className="mt-1 text-xs text-muted-foreground">The student account must already exist and be active.</p>
      </div>
      <Button type="submit" disabled={mutation.isPending}>
