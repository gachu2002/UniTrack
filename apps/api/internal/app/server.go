...
func NewServer(cfg config.Config, db *pgxpool.Pool, logger *slog.Logger) *Server {
	return &Server{cfg: cfg, db: db, logger: logger, rateLimits: newRateLimitStore(), fileStore: newUploadFileStore(cfg)}
}
...
func (s *Server) Handler() http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(protected chi.Router) {
			...
			protected.Patch("/projects/{projectId}/milestones/reorder", s.handleReorderMilestones)
