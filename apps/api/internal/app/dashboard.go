...
func (s *Server) dashboardProjects(ctx context.Context, user User, limit int) ([]ProjectDTO, error) {
	...
	approvedProgressSQL := `(SELECT MAX(pr.reviewed_at) FROM progress_reviews pr JOIN progress_updates pu ON pu.id = pr.progress_update_id JOIN tasks rt ON rt.id = pu.task_id AND rt.project_id = pu.project_id WHERE pu.project_id = p.id AND pr.review_status = 'approved' AND rt.parent_task_id IS NULL)`
	pendingReviewsSQL := `(SELECT COUNT(*) FROM progress_updates pu JOIN tasks rt ON rt.id = pu.task_id AND rt.project_id = pu.project_id WHERE pu.project_id = p.id AND p.status <> 'archived' AND pu.review_status = 'pending_review' AND rt.parent_task_id IS NULL)`
	overdueAssignmentsSQL := `(SELECT COUNT(*) FROM tasks t WHERE t.project_id = p.id AND p.status = 'active' AND t.parent_task_id IS NULL AND t.deadline < current_date AND t.status <> 'done' AND t.official_progress_state <> 'completed')`
	hasAssignmentsSQL := `EXISTS (SELECT 1 FROM tasks t WHERE t.project_id = p.id AND t.parent_task_id IS NULL)`
	where = appendProjectWhere(where, `p.status = 'active'
		AND `+pendingReviewsSQL+` = 0
		AND (`+overdueAssignmentsSQL+` > 0 OR (`+hasAssignmentsSQL+` AND (`+approvedProgressSQL+` IS NULL OR `+approvedProgressSQL+` < now() - interval '7 days')))`)
	args = append(args, limit)
	limitPlaceholder := "$" + strconv.Itoa(len(args))
	attentionOrder := `
		ORDER BY
			` + overdueAssignmentsSQL + ` DESC,
			CASE
				WHEN p.status = 'active'
				 AND ` + hasAssignmentsSQL + `
				 AND (
					` + approvedProgressSQL + ` IS NULL
					OR ` + approvedProgressSQL + ` < now() - interval '7 days'
				 )
				THEN 1 ELSE 0
			END DESC,
			` + approvedProgressSQL + ` ASC NULLS FIRST,
			p.updated_at DESC
...
func (s *Server) dashboardProgressUpdates(ctx context.Context, user User, limit int) ([]ProgressUpdateDTO, error) {
	where := `WHERE t.parent_task_id IS NULL AND pu.review_status = 'pending_review' AND EXISTS (SELECT 1 FROM projects p WHERE p.id = pu.project_id AND p.status <> 'archived')`
	args := []any{limit}
	orderBy := `ORDER BY pu.created_at ASC`

	switch user.Role {
	case RoleTeacher:
		where = `WHERE t.parent_task_id IS NULL AND pu.review_status = 'pending_review' AND EXISTS (SELECT 1 FROM projects p WHERE p.id = pu.project_id AND p.supervisor_id = $1 AND p.status <> 'archived')`
		args = []any{user.ID, limit}
	case RoleStudent:
		where = `WHERE t.parent_task_id IS NULL AND pu.submitted_by = $1 AND EXISTS (SELECT 1 FROM project_members pm WHERE pm.project_id = pu.project_id AND pm.student_id = $1)`
		args = []any{user.ID, limit}
