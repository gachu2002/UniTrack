...
type updateAdminUserRequest struct {
	...
	Role                  *string `json:"role"`
	Status                *string `json:"status"`
	ReplacementSupervisor *string `json:"replacementSupervisorId"`
	ConfirmStudentCleanup bool    `json:"confirmStudentCleanup"`
}
...
type accountTransitionImpactDTO struct {
	RequiresReplacementSupervisor      bool   `json:"requiresReplacementSupervisor"`
	RequiresStudentCleanupConfirmation bool   `json:"requiresStudentCleanupConfirmation"`
	OpenProjectCount                   int    `json:"openProjectCount"`
	ActiveFolderCount                  int    `json:"activeFolderCount"`
	ActiveMembershipCount              int    `json:"activeMembershipCount"`
	ActiveAssignmentCount              int    `json:"activeAssignmentCount"`
	Message                            string `json:"message"`
}

type accountTransitionRequiredResponse struct {
	Error  string                     `json:"error"`
	Code   string                     `json:"code"`
	Impact accountTransitionImpactDTO `json:"impact"`
}

const adminAccountMutationLockKey int64 = 582194305
...
func (s *Server) handleAdminUpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	impact, err := accountTransitionImpactTx(r.Context(), tx, target, nextRole, nextStatus)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "could not inspect account responsibilities")
		return
	}
	replacementSupervisorID := ""
	if impact.RequiresReplacementSupervisor {
		if input.ReplacementSupervisor != nil {
			replacementSupervisorID = strings.TrimSpace(*input.ReplacementSupervisor)
		}
		if replacementSupervisorID == "" || !input.ConfirmStudentCleanup && impact.RequiresStudentCleanupConfirmation {
			writeAccountTransitionRequired(w, impact)
			return
		}
		if replacementSupervisorID == target.ID {
			writeError(w, http.StatusBadRequest, "replacement supervisor must be a different active teacher or admin")
			return
		}
		if err := ensureSupervisorTx(r.Context(), tx, replacementSupervisorID); err != nil {
			writeError(w, http.StatusBadRequest, "replacement supervisor must be an active teacher or admin")
			return
		}
	} else if impact.RequiresStudentCleanupConfirmation && !input.ConfirmStudentCleanup {
		writeAccountTransitionRequired(w, impact)
		return
	}
	...
	if replacementSupervisorID != "" {
		if err := reassignOpenTeacherResponsibilitiesTx(r.Context(), tx, target.ID, replacementSupervisorID); err != nil {
			writeError(w, http.StatusInternalServerError, "could not reassign account responsibilities")
			return
		}
	}
	if impact.RequiresStudentCleanupConfirmation && input.ConfirmStudentCleanup {
		if err := removeActiveStudentResponsibilitiesTx(r.Context(), tx, target.ID); err != nil {
			writeError(w, http.StatusInternalServerError, "could not remove active student responsibilities")
			return
		}
	}
...
func (s *Server) handleAdminSetUserPassword(w http.ResponseWriter, r *http.Request) {
	...
	if err := insertActivityLogTx(r.Context(), tx, actor.ID, "admin.user_password_set", "user", userID, map[string]any{"email": target.Email, "passwordSet": true}); err != nil {
		writeError(w, http.StatusInternalServerError, "could not record account change")
		return
	}
	if err := tx.Commit(r.Context()); err != nil {
...
func accountTransitionImpactTx(ctx context.Context, tx pgx.Tx, target User, nextRole string, nextStatus string) (accountTransitionImpactDTO, error) {
	impact := accountTransitionImpactDTO{}
	losesSupervisorCapability := (target.Role == RoleTeacher || target.Role == RoleAdmin) && (nextStatus != "active" || (nextRole != RoleTeacher && nextRole != RoleAdmin))
	losesStudentCapability := target.Role == RoleStudent && (nextStatus != "active" || nextRole != RoleStudent)

	if losesSupervisorCapability {
		if err := tx.QueryRow(ctx, `SELECT COUNT(*) FROM projects WHERE supervisor_id = $1 AND status <> 'archived'`, target.ID).Scan(&impact.OpenProjectCount); err != nil {
			return impact, err
		}
		if err := tx.QueryRow(ctx, `SELECT COUNT(*) FROM course_sections WHERE owner_teacher_id = $1 AND status = 'active'`, target.ID).Scan(&impact.ActiveFolderCount); err != nil {
			return impact, err
		}
		impact.RequiresReplacementSupervisor = impact.OpenProjectCount > 0 || impact.ActiveFolderCount > 0
	}
	if losesStudentCapability {
		...
		`, target.ID).Scan(&impact.ActiveMembershipCount); err != nil {
			return impact, err
		}
		if err := tx.QueryRow(ctx, `
			SELECT COUNT(*)
			...
			WHERE ta.student_id = $1
			  AND p.status IN ('active', 'on_hold')
		`, target.ID).Scan(&impact.ActiveAssignmentCount); err != nil {
			return impact, err
		}
		impact.RequiresStudentCleanupConfirmation = impact.ActiveMembershipCount > 0 || impact.ActiveAssignmentCount > 0
	}
	impact.Message = accountTransitionMessage(impact)
	return impact, nil
}

func accountTransitionMessage(impact accountTransitionImpactDTO) string {
	switch {
	case impact.RequiresReplacementSupervisor && impact.RequiresStudentCleanupConfirmation:
		return "Choose a replacement supervisor and confirm removal from active student work before changing this account."
	case impact.RequiresReplacementSupervisor:
		return "Choose an active teacher or admin to take over this account's open projects and active folders."
	case impact.RequiresStudentCleanupConfirmation:
		return "Confirm removal from active project memberships and assignments before changing this student account."
	default:
		return "No active account-transition work is required."
	}
}

func writeAccountTransitionRequired(w http.ResponseWriter, impact accountTransitionImpactDTO) {
	writeJSON(w, http.StatusConflict, accountTransitionRequiredResponse{
...
func reassignOpenTeacherResponsibilitiesTx(ctx context.Context, tx pgx.Tx, targetUserID string, replacementSupervisorID string) error {
	if _, err := tx.Exec(ctx, `
		UPDATE projects
		SET supervisor_id = $1
		WHERE supervisor_id = $2
		  AND status <> 'archived'
	`, replacementSupervisorID, targetUserID); err != nil {
		return err
	}
	_, err := tx.Exec(ctx, `
		UPDATE course_sections
		SET owner_teacher_id = $1
		WHERE owner_teacher_id = $2
		  AND status = 'active'
	`, replacementSupervisorID, targetUserID)
	return err
}

func removeActiveStudentResponsibilitiesTx(ctx context.Context, tx pgx.Tx, targetUserID string) error {
	_, err := tx.Exec(ctx, `
...
func lockAdminAccountMutationTx(ctx context.Context, tx pgx.Tx) error {
	_, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock($1)`, adminAccountMutationLockKey)
	return err
}
...
func insertProjectActivityLogTx(ctx context.Context, tx pgx.Tx, actorID string, projectID string, action string, entityType string, entityID string, metadata any) error {
	metadataJSON, err := json.Marshal(metadata)
	if err != nil {
		return err
	}
	_, err = tx.Exec(ctx, `
		INSERT INTO activity_logs (actor_id, project_id, action, entity_type, entity_id, metadata)
		VALUES ($1, $2, $3, $4, $5, $6::jsonb)
	`, actorID, projectID, action, entityType, entityID, string(metadataJSON))
	return err
}
