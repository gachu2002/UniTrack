...
func (s *Server) handleUpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	var currentTitle, currentStatus, currentPriority, currentOfficialProgressState string
	var currentDescription, currentMilestoneID sql.NullString
	var currentDeadline sql.NullTime
	if err := tx.QueryRow(r.Context(), `
		SELECT title, description, status, priority, deadline, milestone_id::text, official_progress_state
		FROM tasks
	...
	description := nullString(currentDescription)
	...
	officialProgressState := currentOfficialProgressState
	...
	if currentDeadline.Valid {
		deadlineValue = currentDeadline.Time.Format("2006-01-02")
	}
	...
	milestoneID := ""
	if currentMilestoneID.Valid {
		milestoneID = currentMilestoneID.String
	}
	...
	if err := ensureMilestoneInProjectTx(r.Context(), tx, projectID, milestoneID); err != nil {
	...
	if status == "done" || officialProgressState == "completed" {
		...
		if hasPendingReview {
			writeError(w, http.StatusConflict, "review pending submissions before completing this assignment")
			return
		}
	}
	...
	result, err := tx.Exec(r.Context(), `
		...
		WHERE id = $9 AND project_id = $10 AND parent_task_id IS NULL
	...
	if input.AssigneeIDs != nil {
		if _, err := tx.Exec(r.Context(), `DELETE FROM task_assignees WHERE project_id = $1 AND task_id = $2`, projectID, taskID); err != nil {
...
func (s *Server) handleCreateProgressUpdate(w http.ResponseWriter, r *http.Request) {
	...
	defer func() { _ = tx.Rollback(r.Context()) }()
	if !s.requireProjectLifecycleTx(w, r.Context(), tx, projectID, "submitting work", projectAcceptsStudentSubmissions) {
		return
	}
	...
	assigned, err = isTaskAssignedToActiveStudentTx(r.Context(), tx, projectID, taskID, user.ID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "could not verify task assignment")
		return
	}
...
func (s *Server) handleReviewProgressUpdate(w http.ResponseWriter, r *http.Request) {
	...
	reviewComment := strings.TrimSpace(input.ReviewComment)
	if reviewRequiresComment(status) && reviewComment == "" {
		writeError(w, http.StatusBadRequest, "review comment is required when returning work for revision")
		return
	}
	...
	defer func() { _ = tx.Rollback(r.Context()) }()
	if !s.requireProjectLifecycleTx(w, r.Context(), tx, projectID, "reviewing submissions", projectAcceptsReviews) {
		return
	}
	allowed, err = canManageProjectTx(r.Context(), tx, user, projectID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "could not verify reviewer access")
		return
	}
	if !allowed {
		writeError(w, http.StatusForbidden, "only the supervising teacher or an admin can review progress")
	...
	if err := tx.QueryRow(r.Context(), `
		SELECT pu.task_id::text, pu.review_status, pu.created_at
		FROM progress_updates pu
		JOIN tasks t ON t.id = pu.task_id AND t.project_id = pu.project_id
		WHERE pu.id = $1 AND pu.project_id = $2 AND t.parent_task_id IS NULL
		FOR UPDATE OF pu
	...
	if submittedAt.Valid {
		...
		if err := tx.QueryRow(r.Context(), `SELECT EXISTS (SELECT 1 FROM progress_updates WHERE project_id = $1 AND task_id = $2 AND created_at > $3)`, projectID, taskID, submittedAt.Time).Scan(&newerSubmissionExists); err != nil {
	...
	_, err = tx.Exec(r.Context(), `
		...
		VALUES ($1, $2, $3, $4, $5)
	`, updateID, user.ID, status, optionalString(reviewComment), optionalString(officialState))
	if err != nil {
...
func isTaskAssignedToActiveStudentTx(ctx context.Context, tx pgx.Tx, projectID string, taskID string, studentID string) (bool, error) {
	var id string
	err := tx.QueryRow(ctx, `
		SELECT ta.student_id::text
		FROM task_assignees ta
		JOIN project_members pm ON pm.project_id = ta.project_id AND pm.student_id = ta.student_id
		JOIN users u ON u.id = ta.student_id
		WHERE ta.project_id = $1
		  ...
		  AND u.role = 'student'
		  AND u.status = 'active'
		FOR UPDATE OF ta, pm, u
	`, projectID, taskID, studentID).Scan(&id)
	if err != nil {
		if isNoRows(err) {
			return false, nil
		}
...
func reviewRequiresComment(reviewStatus string) bool {
	return reviewStatus == "needs_changes" || reviewStatus == "rejected"
}
