...
Cache dữ liệu server và xử lý dữ liệu cũ trên giao diện & Tự viết fetch/cache, Redux Toolkit Query & TanStack Query, Zustand & TanStack Query phù hợp invalidation theo route/dữ liệu; Zustand chỉ dùng cho session state nhỏ \\
...
Phần giao diện được xây dựng bằng React, TypeScript và Vite. React phù hợp với các màn hình như dashboard, Workspace, hồ sơ dự án và trang nhiệm vụ, nơi dữ liệu thay đổi theo vai trò người dùng, trạng thái dự án và kết quả trả về từ API. TypeScript giúp thống nhất kiểu dữ liệu giữa giao diện và server, giảm lỗi khi thay đổi các đối tượng như dự án, nhiệm vụ, bài nộp, tài nguyên hoặc minh chứng. Vite được dùng để chạy môi trường phát triển và tạo bản build nhanh cho ứng dụng SPA \cite{react,typescript,vite}.
...
Giao diện phải xử lý dữ liệu cũ. Nếu người dùng đang mở form tạo nhiệm vụ trong lúc dự án bị chuyển sang \code{completed} ở session khác, server sẽ từ chối request và giao diện cần tải lại dữ liệu để ẩn thao tác không còn hợp lệ.
...
PostgreSQL là lớp bảo vệ quan trọng của hệ thống. Ngoài việc lưu dữ liệu, UniTrack cần giữ đúng quan hệ giữa dự án, thành viên, mốc kế hoạch, nhiệm vụ, bài nộp, duyệt bài, tài nguyên và minh chứng. Các khái niệm khóa ngoại, ràng buộc toàn vẹn, transaction và phục hồi khi lỗi là nền tảng của hệ quản trị cơ sở dữ liệu quan hệ \cite{silberschatz2020database}. PostgreSQL hỗ trợ khóa ngoại, chỉ mục duy nhất, transaction và trigger, phù hợp với các ràng buộc mà server cũng kiểm tra ở tầng ứng dụng \cite{postgresql}.
...
UniTrack dùng cookie-based session. Khi đăng nhập thành công, server sinh token ngẫu nhiên, lưu token hash trong bảng \code{sessions} và gửi cookie HttpOnly về browser. Cách làm này tránh lưu access token trong local storage. Các cấu hình \code{Secure}, \code{SameSite}, CORS, trusted proxy khi khai báo và bootstrap admin được parse nghiêm ngặt lúc khởi động; giá trị sai định dạng hoặc cấu hình production nguy hiểm làm API thoát lỗi thay vì chạy với trạng thái không an toàn.

Minh chứng trong môi trường phát triển được lưu trên local filesystem với quyền file riêng tư. Khi triển khai, server được thiết kế để dùng Cloudflare R2 private bucket. File không public trực tiếp; người dùng tải qua API để server kiểm tra quyền xem dự án trước khi truyền nội dung. Luồng upload tạo job dọn lỗi trước khi ghi file, rồi hoàn tất job trong transaction lưu metadata; nếu metadata lỗi, job còn lại giúp dọn file mồ côi. Luồng xóa metadata cũng tạo job dọn lỗi, thử xóa ngay sau commit và để job còn pending được xử lý lại khi API khởi động; hệ thống chưa có worker định kỳ cho hàng đợi này \cite{cloudflarer2}.

\section{Kiểm thử và triển khai}

Server dùng Go test để kiểm tra các ràng buộc nghiệp vụ quan trọng như session, phân quyền, vòng đời dự án, bài nộp, duyệt bài, minh chứng và trigger cơ sở dữ liệu. Frontend dùng Playwright cho các browser flow có rủi ro cao như đăng nhập, phân quyền, khả năng truy cập, dashboard và dữ liệu cũ trên giao diện \cite{playwright}.
...
Server & Go, chi, pgx & API REST, middleware, phân quyền, truy vấn SQL và transaction \\
...
Kiểm thử & Go test, Playwright & Kiểm tra nghiệp vụ server và browser flow quan trọng \\
